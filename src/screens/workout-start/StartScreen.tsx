...
const StartScreenComponent = memo(
	({ workoutData }: StartScreenComponentProps) => {
		useKeepAwake();
		...
		const workoutOver = useCallback(async () => {
			...
			try {
				await db
					.insert(record)
					.values({ ...rest, duration: targetTime });
				router.navigate("/");
			} catch (err) {
				console.log(err);
			}
...
const StartScreen = memo(() => {
	...
	return <StartScreenComponent workoutData={workoutData} />;
});
