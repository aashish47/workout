...
const MultiModeCounterInput = memo(
	({
		mode,
		timer,
		timerValue,
		setWorkoutData,
	}: MultiModeCounterInputProps) => {
			...
			setTime((prevTime) => {
				...
				const newMinutes =
					highlight === "seconds"
						...
						: highlight === "minutes"
							? prevTime.minutes + 1
							: prevTime.minutes + (newSeconds === 60 ? 1 : 0);
				return {
					minutes: newMinutes,
		...
		const incrementCounter = useCallback(() => {
			setCounter((prevCounter) =>
				prevCounter < 99 ? prevCounter + 1 : prevCounter,
			);
		}, []);
		...
		const handleTextPress = useCallback(
			(type: "minutes" | "seconds" | "counter") => {
				setHighlight((prevHighlight) =>
					prevHighlight === type ? null : type,
				);
			},
			[],
		);
